...
String watchSurrounding(){
  ...
  head.write(150); //angle position of servo
  delay(100);
  ...
  //head.write(170); //was 170
  head.write(90); //was 90
  delay(100);
  centerScan = watch();
  if(centerScan<distanceLimit){
    stop();
  ...
  //head.write(0); //was 0
  //head.write(90); //head is facing forward again, was 90
  delay(300);
...
void obstacleAvoidance(){
  ...
  if(numCycles >= LPT){ //stops to check the surrounds every LPT cycles
    ...
    if(obstacleSign == "100"){ //move slightly right
      setSpeed(fastSpeed,speed);
      forward();
      delay(turnTime);
      stop();
    }
    else if(obstacleSign == "001"){ //move slightly left
      setSpeed(speed,fastSpeed);
      forward();
      delay(turnTime);
      stop();
    }
    else if(obstacleSign == "011"){ //move right and back
	    left();
		  setSpeed(fastSpeed,speed);
      delay(turnTime);
      stop();
    }
    else if((obstacleSign == "111") || (obstacleSign == "010")){ //move backward
	    backward();
		  setSpeed(speed,speed);
      delay(turnTime);
      stop();
    } 
    else if(obstacleSign=="110"){ //move left and back
      right();
      setSpeed(speed,fastSpeed);
      delay(turnTime);
      stop();
